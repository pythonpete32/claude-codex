...
import type { ActiveSession, FileLogEntry } from "@claude-codex/log-processor";
	...
	async getAllSessions(options: {
		...
		if (active) {
			...
			const activeIds = new Set(
				activeSessions.map((s: ActiveSession) => s.sessionId),
			);
	...
	private async scanAllSessions(): Promise<SessionInfo[]> {
		try {
			...
			const activeMap = new Map(
				activeSessions.map((s: ActiveSession) => [s.sessionId, s] as const),
			);
