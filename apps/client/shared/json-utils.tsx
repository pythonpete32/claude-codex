...
const JsonTree: React.FC<{
	...
	if (data === null || typeof data !== "object") {
		return (
			<span className={cn("font-mono text-sm", getValueColor(data))}>{JSON.stringify(data)}</span>
		);
	}
			...
			<div className="font-mono text-sm">
				...
				{isExpanded && (
					<div className="ml-4 border-l border-gray-200 pl-2">
						...
						{data.length > 10 && (
							<div className="text-gray-500 italic">... and {data.length - 10} more items</div>
						)}
					</div>
				)}
			</div>
		...
		<div className="font-mono text-sm">
			...
			{isExpanded && (
				<div className="ml-4 border-l border-gray-200 pl-2">
					...
					{keys.length > 10 && (
						<div className="text-gray-500 italic">... and {keys.length - 10} more keys</div>
					)}
				</div>
			)}
		</div>
