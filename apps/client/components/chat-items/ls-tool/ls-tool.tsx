...
export const LsTool: React.FC<LsToolProps> = ({ toolUse, toolResult, status, timestamp, className }) => {
	...
	const directories = entries.filter((f) => f.type === "directory").sort((a, b) => a.name.localeCompare(b.name))
	const regularFiles = entries.filter((f) => f.type === "file").sort((a, b) => a.name.localeCompare(b.name))
	...
	const formatLsOutput = () => {
		...
		return sortedFiles.map((file) => {
			...
			return (
				<div key={file.name} className={cn("hover:bg-gray-800/50 px-1 font-mono text-sm", file.hidden && "opacity-60")}>
