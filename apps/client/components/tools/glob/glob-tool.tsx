...
export const GlobTool: React.FC<GlobToolProps> = ({
	...
	const formatFileResults = () => {
		...
		const filesByDir = results.reduce(
			(acc, filePath) => {
				...
				{Object.entries(filesByDir).map(([dir, files]) => (
					<div key={dir} className="border border-gray-700 rounded-md p-3 bg-gray-900/50">
						<div className="flex items-center gap-2 mb-2">
							...
							<span className="font-mono text-sm text-blue-400">{dir === "." ? "Current directory" : dir}</span>
							...
							{files.map((filePath, index) => {
								...
								return (
									...
									>
										...
										<span className="text-gray-300 hover:text-white">{fileName}</span>
									</div>
								)
							})}
						</div>
	...
	const output = (
		<div>
			...
			<div className="flex items-center gap-2 mb-4">
				...
				{ui.searchTime && ui.searchTime > 0 && <span className="text-xs text-gray-500">({ui.searchTime}ms)</span>}
			</div>

			{/* File results */}
			<div className="max-h-96 overflow-y-auto">{formatFileResults()}</div>
		</div>
	)
