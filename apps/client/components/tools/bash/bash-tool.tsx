			...
			<div className="flex items-center justify-between border-b border-gray-700 p-4">
				<div className="flex items-center gap-x-3">
					...
					<div className="flex items-center gap-2 text-gray-300">
						...
						<span className="text-sm font-medium">{elevated ? "sudo bash" : "bash"}</span>
					</div>
				</div>
				...
				<div className="flex items-center gap-2">
					<Badge variant="outline" className={getStatusColor()}>
						{getStatusIcon()}
						<span className="ml-1 capitalize text-xs">{status.original || normalizedStatus.replace("_", " ")}</span>
					</Badge>
					{duration && (
			...
			<div className="p-4 font-mono text-sm">
					...
					<div className="flex-1 flex items-center justify-between">
						...
						<div className="flex items-center gap-1 ml-4">
							...
							{onRerun && (
								...
								>
									{normalizedStatus === "running" ? <Square className="h-3 w-3" /> : <Play className="h-3 w-3" />}
								</Button>
							)}
						</div>
					</div>
				...
				{displayOutput && (
					<AnimatedSpan delay={800} className="block">
						<div className={cn("whitespace-pre-wrap break-words", hasError ? "text-red-300" : "text-gray-300")}>
							{animated && (normalizedStatus === "running" || normalizedStatus === "completed") ? (
								<TypingAnimation duration={20} delay={1000} className={hasError ? "text-red-300" : "text-gray-300"}>
									{displayOutput}
								</TypingAnimation>
							) : (
								displayOutput
			...
			{(timestamp || metadata) && (
				<div className="border-t border-gray-700 px-4 py-2">
					<div className="flex items-center justify-between text-xs text-gray-500">
						...
						{metadata?.executionId && <div className="text-gray-600">ID: {metadata.executionId}</div>}
					</div>
				</div>
			)}
		</div>
