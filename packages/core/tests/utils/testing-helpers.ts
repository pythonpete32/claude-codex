...
/**
 * Test utilities for fixture-based parser testing
 */
export class TestingHelpers {
   ...
   */
  static validateBaseToolProps(props: BaseToolProps): void {
   ...
   */
  static validateToolStatus(status: ToolStatus): void {
   ...
   */
  static validateUuidCorrelation(
    toolCall: LogEntry,
    toolResult?: LogEntry
  ): void {
  ...
  static deepCompareWithPath(
    ...
    if (Array.isArray(actual) && Array.isArray(expected)) {
      ...
      for (let i = 0; i < actual.length; i++) {
        this.deepCompareWithPath(actual[i], expected[i], `${path}[${i}]`);
      }
      return;
    }
   ...
   */
  static createMockLogEntry(overrides: Partial<LogEntry> = {}): LogEntry {
    return {
      uuid: 'test-uuid-123',
   ...
   */
  static createMockBaseToolProps(
   ...
   */
  static generateTestDescription(toolCall: LogEntry): string {
    try {
      const content =
...
export function setupCustomMatchers() {
  expect.extend({
    toHaveValidToolProps(received: BaseToolProps) {
      try {
        TestingHelpers.validateBaseToolProps(received);
        return {
    ...
    toHaveValidStatus(received: ToolStatus) {
      try {
        TestingHelpers.validateToolStatus(received);
        return {
    ...
    toMatchFixtureOutput(received: unknown, expected: unknown) {
      try {
        TestingHelpers.deepCompareWithPath(received, expected);
        return {
    ...
    toHaveCorrelatedUuids(received: {
      ...
      try {
        TestingHelpers.validateUuidCorrelation(
          received.toolCall,
          received.toolResult
        );
        return {
