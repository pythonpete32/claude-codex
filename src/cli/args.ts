...
export function parseArgs(): ClaudeMaxOptions {
  ...
  if (args.length === 0 || args.includes("--help") || args.includes("-h")) {
    console.log(
      colors.bright("Claude Code SDK - Max Subscription") + "\n",
    );
    console.log("Usage:");
    console.log('  npx tsx claude-sdk-max.ts "Your prompt here" [options]\n');
    console.log("Options:");
    console.log(
      "  --max-turns <n>      Maximum conversation turns (default: 1)",
    );
    console.log(
    ...
    console.log(
      "  --debug              Show debug information for all messages",
    );
    ...
    console.log(
      '  npx tsx claude-sdk-max.ts "Fix the bug in app.js" --max-turns 3',
    );
    console.log(
      '  npx tsx claude-sdk-max.ts "Refactor this code" --allowed-tools Read,Write',
    );
  ...
  for (let i = 1; i < args.length; i++) {
    switch (args[i]) {
      case "--max-turns":
        options.maxTurns = parseInt(args[++i], 10) || 1;
        break;
      case "--output-format":
