...
export class SessionScanner {
	...
	async getAllSessions(options: {
		...
		if (project) {
			filteredSessions = filteredSessions.filter((s) => s.projectPath.includes(project));
		}
	...
	private async createSessionInfo(
		...
		try {
			...
			console.error(`Error creating session info for ${entry.sessionId}:`, error);
			return null;
		}
	}
	 ...
	 */
	private async getSessionCreatedTime(filePath: string): Promise<string | null> {
		try {
			const { readFile } = await import("node:fs/promises");
			const content = await readFile(filePath, "utf-8");
	...
	private async checkForToolUsage(filePath: string): Promise<boolean> {
		try {
			...
			return content.includes('"type":"tool_use"') || content.includes('"tool_use_id"');
		} catch {
			return false;
		}
	}
