/**
 * Fixture data and exports for grep-tool
 * @packageDocumentation
 * @module @dao/codex-chat-item-grep-tool/fixtures
 */

import fixturesData from "./fixtures.json" with { type: "json" };
import { parseGrepTool } from "./parsers";
import type { GrepFixtureData, GrepFixturesMetadata } from "./types";

/**
 * Validated fixtures from the fixtures.json file.
 * Only includes fixtures with complete data that pass validation.
 */
export const ValidatedFixtures: GrepFixturesMetadata = (() => {
	// Validate the imported fixtures data structure
	const metadata = fixturesData as GrepFixturesMetadata;
...
export namespace AllFixtures {
	/**
	 * First fixture - successful grep with matches.
	 */
	export const successfulSearch: GrepFixtureData = ValidatedFixtures.fixtures[0];

	/**
	 * Find a failed grep fixture from the collection.
	 */
	export const failedSearch: GrepFixtureData =
		...
		}) || ValidatedFixtures.fixtures[0]; // Fallback to first if no failed search found

	/**
	 * Find a fixture with no matches.
	 */
	export const emptySearch: GrepFixtureData =
		ValidatedFixtures.fixtures.find((fixture) => {
			const result = fixture.toolResult.toolUseResult;
	 ...
	 */
	export const allFixtures: GrepFixtureData[] = ValidatedFixtures.fixtures;
}
